...
def book(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return render(request, 'core/book.html', {'room_name': room_name, 'error': 'Please enter a valid email address.'})

           # --- 3. Guests count check ---
        try:
            guests_count = int(guests)
            if guests_count < 1:
                raise ValueError
        except (ValueError, TypeError):
            return render(request, 'core/book.html', {'room_name': room_name, 'error': 'Please enter a valid number of guests.'})
        ...
        try:
            ...
            return render(request, 'core/book.html', {'room_name': room_name, 'error': 'Invalid dates.'})

        if checkout_date <= checkin_date:
            return render(request, 'core/book.html', {'room_name': room_name, 'error': 'Check-out must be after check-in.'})

        if checkin_date < timezone.now().date():
            return render(request, 'core/book.html', {'room_name': room_name, 'error': 'Check-in cannot be in the past.'})
        # --- end validation ---
